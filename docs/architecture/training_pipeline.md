...
- VH_VV_ratio = sign(VH/VV) * log1p(|VH/VV|) (with division by zero protection → NaN)
- VH_VV_diff = VH - VV

**Motivation**: Captures radar backscatter characteristics sensitive to water surface roughness and vegetation structure  
**Implementation**: `feature_engineering.py:transform()` lines 386-403
...
**Mathematical definitions**:
- Ratio: sign(SAR_band / Optical_index) * log1p(|SAR_band / Optical_index|)
...
Feature selection is integrated directly into the training pipeline via the `TrainingConfig` configuration. This allows users to easily enable and configure feature selection without modifying the training code.

#### Configuration Options
- `feature_selection_enabled` (bool): Set to `True` to enable feature selection (default: `False`)
- `feature_selection_method` (str): Method to use for feature selection (`'groups'`, `'names'`, `'patterns'`, `'indices'`, `'custom'`, `'combine'`) (default: `'groups'`)
- `feature_selection_kwargs` (dict): Keyword arguments for the selected method (default: `{}`)

#### Default Behavior
By default, feature selection is disabled (`feature_selection_enabled = False`), which means all features generated by the feature engineering pipeline are used for training. This maintains backward compatibility with existing code.
...
# The trainer will now use feature selection as configured
```

#### Benefits of Integrated Approach
- **No code changes required**: Simply modify the configuration to enable/disable feature selection
...
4. **Feature Selection Configuration**
   - `feature_selection_enabled`: Boolean to enable/disable feature selection (default: False)
   - `feature_selection_method`: Method for feature selection ('groups', 'names', 'patterns', 'indices', 'custom', 'combine') (default: 'groups')
   - `feature_selection_kwargs`: Dictionary of arguments for the selection method (default: {})
